...
pub struct Tensor<S> where S: RawData {
    id: AtomicId,
...
impl<S> Tensor<S> where S: RawData {
    pub fn new<D>(data: ArrayBase<S, D>) -> Self where D: Dimension {
        Tensor {
            id: AtomicId::new(),
            data: data.into_dyn(),
            op: None,
...
impl<S> fmt::Debug for Tensor<S> where S: Data, S::Elem: fmt::Debug {
