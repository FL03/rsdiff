...
pub trait Monad<U>: Applicative<U> {
    ...
    fn bind<F>(&self, fs: F) -> Self::T where F: FnMut(&Self::C) -> Self::T;
...
impl<T, U> Monad<U> for Box<T> {
    fn bind<F>(&self, mut fs: F) -> Box<U> where F: FnMut(&T) -> Box<U> {
        fs(self)
    }
}

impl<T, U> Monad<U> for Option<T> {
    fn bind<F>(&self, mut fs: F) -> Option<U> where F: FnMut(&T) -> Option<U> {
        match *self {
...
impl<T, U> Monad<U> for Rc<T> {
    fn bind<F>(&self, mut fs: F) -> Rc<U> where F: FnMut(&T) -> Rc<U> {
        fs(self)
    }
}

impl<T, U> Monad<U> for Vec<T> {
    fn bind<F>(&self, mut fs: F) -> Vec<U> where F: FnMut(&T) -> Vec<U> {
        let mut v = Vec::new();
        for x in self {
            v.extend(fs(x));
        }
